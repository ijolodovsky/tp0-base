...
#### Delegación de Conexiones a Threads:
```python
...
#### Sincronización con File Lock:
```python
# server/common/server.py - Protección de operaciones de archivo
def __handle_client_connection(self, client_sock):
    if msg_type == 'BATCH_APUESTAS':
        bets = parse_bet_batch_content(content)
        
        try:
            # Usar file_lock para escritura exclusiva al CSV
            with self.file_lock:
                store_bets(bets)  # Función provista - no thread-safe
...
#### Sincronización con Lock Principal:
```python
...
def __handle_client_connection(self, client_sock):
        ...
        # Usar lock principal para modificar variables compartidas
        with self.lock:
            self.finishedAgencies += 1
            # Verificar condición de sorteo de forma thread-safe
            if self.finishedAgencies == self.expected_agencies and not self.sorteoRealizado:
                self.sorteoRealizado = True
                logging.info("action: sorteo | result: success")
                self.respond_pending_winners()  # Responder consultas en espera
...
#### Notificación de Fin de Apuestas - Cliente:
```go
// client/common/client.go - Secuencia completa del cliente
func (c *Client) StartClientLoop(sigChan chan os.Signal) {
    // 1. Procesar archivo CSV por batches
    if err := c.processCSVFile(); err != nil {
        log.Errorf("action: process_csv | result: fail | client_id: %v", c.config.ID)
        return
    }
    // 2. Notificar fin de apuestas
...
func (c *Client) finishNotification() {
    if err := protocol.SendFinishConfirmation(c.conn, c.config.ID); err != nil {
        log.Errorf("action: send_finish_notification | result: fail", err)
        return
    }
...
func (c *Client) consultWinners() {
    protocol.SendWinnersQuery(c.conn, c.config.ID)
    winners, _ := protocol.ReceiveWinnersList(c.conn)
    log.Infof("action: consulta_ganadores | result: success | cant_ganadores: %d", len(winners))
}
```

#### Protocolo de Mensajes Tipificados:
```go
// client/protocol/protocol.go - Envío de notificaciones
...
#### Manejo del Sorteo en Servidor:
```python
# server/protocol/protocol.py - Identificación de tipos de mensaje
def read_message(sock) -> tuple[str, str]:
    """Lee un mensaje y determina su tipo."""
    header = _read_n_bytes(sock, 2)
    message_length = (header[0] << 8) | header[1]
    text = _read_n_bytes(sock, message_length).decode('utf-8')
    ...
    if text.startswith('FIN_APUESTAS|'):
        ...
        return ('BATCH_APUESTAS', text)  # Batch de apuestas
...
def __handle_client_connection(self, client_sock):
    while True:
        ...
        if msg_type == 'FIN_APUESTAS':
            ...
            agency_id = int(content)
            if not self.sorteoRealizado:
                # Agregar a lista de espera (mantener conexión)
                self.client_connections.append((client_sock, agency_id))
                return
            else:
                # Responder inmediatamente
...
def get_winners_agency(self, agency_id: int) -> list[str]:
    """Obtiene los DNI de los ganadores de una agencia específica."""
    bets = list(load_bets())  # Función provista por la cátedra
    winners_dni = []
    
    for bet in bets:
        if bet.agency == agency_id and has_won(bet):  # Función provista
            winners_dni.append(bet.document)
    
    return winners_dni

def respond_pending_winners(self):
    """Responde a todas las consultas de ganadores en espera."""
    for client_sock, agency_id in self.client_connections:
        try:
            ...
            logging.error(f"Error responding to pending winner query: {e}")
